...
namespace GEH.Entities
{
    ...
    public abstract class Entity
    {
        ...
        public Shader ShaderComponent { get; private set; }
        ...
        {
            NameObject = name;
            ShaderComponent = new Shader(@"shader.vert", @"shader.frag");
            PhysicsComponent = new Physics(this);
            CollisionComponent = new Collision(this);
            CollisionComponent.IsEnabled = false;
            PhysicsComponent.IsEnabled = false;
         
            CameraComponent = new Camera();
            CameraComponent.IsEnabled = false;
            ShaderComponent.LoadShaders();
        }
       public void UpdateUniformInShader(Matrix4 matrix4)
        {
            ShaderComponent.UpdateUniform(matrix4 * Matrix4.CreatePerspectiveFieldOfView(1.3f, 1, 1.0f, 1000.0f));
            ShaderComponent.Use();
        }
       ...
       public void Move(Vector3 moveVector)
        ...
        public virtual void VusialEntity()
        {
            ...
            GL.BufferData(BufferTarget.ElementArrayBuffer, (IntPtr)(GetPoints().ToArray().Length * sizeof(int)), GetPoints().ToArray(), BufferUsageHint.StaticDraw);
